...
pgvector保持当前生产基线。只有在真实数据规模、P50/P95延迟、并发、资源占用和HNSW/IVFFlat/iterative scan等调优结果显示查询瓶颈或业务负载互相影响时，才启动独立向量数据库评估（如Milvus、Qdrant、Weaviate）；先做同语料Benchmark并记录回退、删除、权限过滤和运维成本。ChromaDB只用于不依赖完整PostgreSQL的轻量隔离RAG PoC，不进入生产主线。该门禁与阶段1的[技术栈缺口评估](../阶段1/02-技术方案与实施计划.md#技术栈缺口评估与隔离验证)一致。
...
图中资源、审阅日期和联系方式不是实际审核证据；S18引用核对版本与删除状态，不能仅凭可见链接判定可访问。S12工具写入仍需明确确认及真实业务收据。
