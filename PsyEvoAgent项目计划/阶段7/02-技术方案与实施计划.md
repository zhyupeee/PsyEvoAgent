...
G控制器、BudgetService、final evaluator/holdout、安全基线、Agent/Tool白名单、权限/所有权/同意、保留删除、审计、发布审批皆固定。依赖评估器升级属于G独立校准，不能和E6一起改变。L2写meta_candidates目录、G只读assets、无生产网络/凭证/通知；diff通过也必须独立执行验证，不能只靠静态扫描断言安全。
...
固定历史重放仅给定上下文响应比较；模拟用户可压力测试对话轨迹，但没有真人反事实效力。任何真实长期实验须另获伦理、知情同意、年龄/专业支持和适用公众服务资格。本轮不创建参与者、不训练、不调用收费模型、不发通知。
...
development供优化；selection选择候选；regression执行固定保护检查；holdout封存，在协议规定次数下由评价服务读取。按场景/来源会话族分割，避免相邻轮次泄漏。真实许可或用途撤回时更新材料资格，旧实验保留失效说明，不能继续给优化器使用。
...
RSI-S7-T02与S7-T04的原任务关联表示共同产出，不表示必须先完成S7-T04才可装配对照。详细执行顺序见本阶段04；没有先导或资格时，记录正式实验blocked及原因，仍可交付工程机制证据，不填写效果通过。
