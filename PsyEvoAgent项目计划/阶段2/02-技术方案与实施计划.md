...
UserDependencyAssessor只处理获准当前输入、本站事件、所选记录、自愿自评和更正；不读取设备其他应用。长期趋势在已有worker生成，实时只取未过期、版本有效的最小Profile；无授权仅在当前run处理当前直接相关表达，不落长期记录。
...
| reassurance_loop | 同一担忧反复求保证、得到保证后很快再问 | 区分澄清新事实和重复确认；没有跨会话许可不跨会话推断 |
...
在已有id/kind/source_refs/generation/status/attempt/lease_until/cancelled_at/error_code上补version、owner/purpose绑定、grant/consent版本、available_at、max_attempts、deadline_at、budget_ref、lease_owner及递增lease_token。领取用短事务行锁/CAS，续租与完成必须匹配lease_token；租约回收后旧Worker不得提交。租约不是数据访问授权，每次读取/调用/提交仍核来源与用途。job重试不重置run/experiment总预算、attempt或deadline。

业务链：`用户允许候选提取→同事务保存job→阶段2 Worker轮询领取→复核来源→LangMem→校验→candidate→用户确认→memories`。阶段4内容链为`批准版本＋job＋outbox同库事务→发布通知→Kafka→Worker→候选索引generation→人工发布`。outbox早已有设计；阶段2无消息双写时不要求提前部署Kafka。正式记忆确认是同步业务事务，不用FastAPI进程内BackgroundTasks假装耐久提交；删除传播、导出、纵向评价、L1/L2均依需要复用持久job。
...
| context_grants | id、user_id、source_type/id/version、purpose(current_run/candidate_extraction/personalization)、run_id可空、expires_at、revoked_at、version |
...
以上业务表使用同一PostgreSQL，LangMem不另建自动写Store作为第二主存。候选中的系统推断只能保持候选；用户确认为偏好也必须标其来源与用途。运行摘要、用户核对摘要和长期记忆分别保存，不能互换状态。

阶段5A可用MemoryCandidateArtifact封装本阶段候选，绑定现有candidate ID/version、source_refs、grant、run/job及generation；不得新增第二memory store或改写正式记忆所有权。授权来源→LangMem/Memory Workflow→candidate→来源/grant/schema检查→用户确认→memories始终不变。单Support读取和提取预算先在本阶段成立，角色拆分与各角色子预算留5A，不提前引入独立Memory Agent。
...
stateDiagram-v2
 ...
 actionable --> unknown: 过期/撤权/更正失效
```

actionable仅表示值得调整帮助方式，不是诊断。profile始终保存window、baseline_ref、coverage、sufficiency、反证、待澄清、expires_at与assessor_version。基础状态可实时核明确诉求；趋势基线只有在许可、覆盖和可比时区满足时估计。阈值/权重/窗口/模型confidence未校准时不产生数值等级，初始规则优先依赖可核对自报与保守澄清。LLM confidence不得当概率、权限或临床风险。
...
| POST `/api/v1/memory-candidates` | grant_id、source_refs、Idempotency-Key | 202 job_id；处理前未授权403；来源版本变化409 |
| GET `/api/v1/memory-candidates` | status/cursor | 仅本人候选及来源、类型、生成状态 |
| POST `/api/v1/memory-candidates/{id}/decision` | decision(confirm/edit_confirm/reject/current_run)、可选edited_content、expected_version；current_run须附run_draft_id | memory_id或绑定该draft的grant_id/拒绝状态；相同决定幂等，已启动run或过期/旧候选409 |
...
创建当前run资料授权时，调用阶段1已有POST `/api/v1/run-drafts`取得draft ID → POST `/api/v1/context-grants`绑定该ID及选定资料版本 → POST `/api/v1/runs/{id}/start`提交用户输入和grant_ids。current_run授权只可绑定本人未启动draft；开始前、实际读取前和写入前重查，run终止/取消/到期后失效。候选卡“仅本次”具体显示为“仅用于下一次回应”，若上一run已结束则新建draft，不能追溯授权过去调用。无关自由笔记不会随会话默认发送。AI整理输出单独为候选，不原位覆盖用户记录。

### 接口与事务

| 接口 | 输入→输出 | 校验与异常 |
|---|---|---|
| GET `/api/v1/me/dependency-profile` | 可选as_of→本人有效Profile及可见Evidence引用 | 未授权返回disabled/无资料，非403封禁对话；已过期status=stale、sufficiency=unknown；不返回其他人的资料 |
| POST `/api/v1/dependency-assessments` | source_refs、consent_ref、expected_profile_version→202 job_id | 只允许本人已有来源，缺scope403/旧版本409；生产自动worker用内部身份，不任意拼用户ID |
...
optional_profile的ConsentRecord必须同时满足来源级dependency_assessment grant；它是context_grants新增用途，保留旧枚举并显式迁移。无该许可时仅当次current_run合法上下文可用，不能保存跨会话画像。Memory仍使用原memory-candidates/decision和context_grants，assessor不拥有正式写入权。DependencyEvidence/Profile可为PostgreSQL独立业务表，引用现有notes/messages/memories，不复制全部原文；共享skill注册表没有个人记忆写入口。提交以source/consent/profile版本比较，撤权/取消/generation变化则丢弃结果。
...
- **撤回用途**：停止该用途的后续读取、候选和写入；保留本人原始记录，界面另提供删除。
...
S2-T04/S2-T05/S2-T06在原有记忆链上补齐提交门禁；验收S2-A11。LangMem提取/摘要独立于阶段6采用哪种prompt optimizer，仍只产候选；确认时以短事务再核owner/source/grant/consent/schema/generation/候选版本及用户选择，同一候选重复确认至多一次正式写入，不用进程内BackgroundTasks替代耐久提交。
...
可选11维画像及unknown/反证/保护因素详见[画像与证据合同](02-技术方案与实施计划.md)；无optional_profile用途不加载长期画像，仍可依据当次合法输入、明确诉求、拒绝与纠正选择已批准支持方式，详见[阶段3 L0](../阶段3/02-技术方案与实施计划.md)。自评与引用该自评的Profile不是两份独立证据，重连/多标签页/重试不增加主动发起。
...
Memory、profile、共享经验和研究案例分别核用途。LangMem只产candidate，确认时再查owner/source/grant/schema/generation及候选版本；正式memory由业务事务唯一写入。修订、更正、撤权同时使Artifact、Review pass、摘要、索引、缓存、checkpoint、导出及研究资格失效。保留来源但遗忘单条记忆时使用最小抑制标记；回滚不能覆盖用户新纠正，trace和共享skill不成为第二资料库。
...
配置候选：`rest_window`、`rest_max_per_session=1`、`cadence_lookback_days=14`、`cadence_min_active_days=5`、用户允许的用途/提醒、事件停止状态。当地日期由服务器当前UTC和已核对IANA时区计算，记录时区变化后暂停规律推断；跨午夜采用环形时段比较或直接放弃低置信规律，不能把23:30与00:30算成相距23小时。
...
增量任务按下表登记；原S*-T*含义保持，页内Txx为本阶段基础任务的局部简称。每项M以内，模型分类作为最后接入项，不阻塞规则与UI。恢复以当前业务授权为准；旧checkpoint不能覆盖新更正。关闭可选assessor不影响原笔记、睡眠、备忘、确认记忆与Support。
...
| RSI-S2-T02 <a id="rsi-s2-t02"></a> 接个人用途与来源映射 | [S2-T01](02-技术方案与实施计划.md)、[S2-T04](02-技术方案与实施计划.md) | RSI-S2-T01 | U05/U08；consents/context_grants、候选用途隔离 | [RSI-S2-A01](03-测试与验收标准.md)、[RSI-S2-A03](03-测试与验收标准.md)、[RSI-S2-A04](03-测试与验收标准.md)；共同S2-A10、S2-A11 |
...
| RSI-S2-T05 <a id="rsi-s2-t05"></a> 接删除、更正传播与到期 | [S2-T06](02-技术方案与实施计划.md) | RSI-S2-T04 | U06/U08；deletion_jobs、suppressions、Artifact失效 | [RSI-S2-A05](03-测试与验收标准.md)、[RSI-S2-A06](03-测试与验收标准.md)；共同S2-A10、S2-A11 |
| RSI-S2-T06 <a id="rsi-s2-t06"></a> 执行fresh-session记忆链验证 | [S2-T08](02-技术方案与实施计划.md) | RSI-S2-T05 | U08；候选确认、真实保存/检索/修订收据 | [RSI-S2-A01](03-测试与验收标准.md)、[RSI-S2-A07](03-测试与验收标准.md)；共同S2-A10、S2-A11 |
...
|---|---|---|
| DependencyEvidence | dimension、evidence_kind=utterance/event/self_report/correction、source_refs（逐项含source_version）、stance=support/counter/ambiguous、alternative_explanations、event_time、expires_at | 引用与必要短片段，用户可更正；来源撤权使派生链失效 |
| DependencyProfile | dimensions、window、sufficiency=unknown/limited/conflicting/sufficient、trend=unknown/stable/increasing/decreasing、counter_refs、protective_factors、clarifications、suggested_actions、assessor_version、expires_at | 多维产品适配状态，非诊断分数；明确区分观察/解释；不向学校/管理员自动展示 |
...
| [langchain-ai/langmem](https://github.com/langchain-ai/langmem/tree/9d033b47d9ce53e37e92c92241b0496c0278932e)<br>`9d033b47d9ce53e37e92c92241b0496c0278932e`<br>2026-09-09T06:44:42Z | src/langmem/knowledge/extraction.py; src/langmem/prompts/optimization.py; tests/short_term/test_summarization.py | MIT；**接口直接复用候选，实施时兼容验收** | 保留已规划的提取/摘要；create_prompt_optimizer是低成本固定L1替代，不是自证L2；候选正式写入仍由业务确认 | S/M：schema/授权/预算/版本4类；LangChain结构化提取与人工候选 |
...
四种用途分离：当前服务必要上下文只处理本次必要信息；跨会话画像可选且默认关闭；长期记忆由用户逐条确认；研究/优化案例另行同意、资格筛选与去标识审查。拒绝后三种非必要处理仍提供基础支持和固定安全功能。不得将敏感资料的所有处理都偷换为“服务必要”。个人资料不进入公共知识/共享技能/学校分析页；校方身份本身不赋予画像访问权。

| 数据类 | 规划保留/过期规则 | 删除与限制 |
|---|---|---|
| 未授权持久化的当次评估上下文 | run终止即清除临时副本；法定必要审计仅留最小元数据 | 不落长期profile；provider已接收内容不能假称本地取消可收回 |
| 可选事件/画像 | 合成开发建议事件7日、个人聚合30日、画像7日到期；均为工程假设，真实使用前由G批准并告知，未批准不得持久采集 | TTL不是心理状态阈值；过期变unknown，不能自动保留“高风险标签”；更正/撤权即失效 |
| 用户原始记录、确认记忆 | 沿原业务明确保存/删除规则；存储期限与服务范围在实际试用前冻结 | 删除源→画像/evidence/候选/索引/cache/checkpoint/导出失效；正式行动独立状态按原合同处理 |
| 研究材料、实验产物 | 逐协议写明结束日与复核日；合成开发包与真实材料分开；期限缺失阻止真实数据导入 | 去标识仍可能是个人信息，撤回后停止使用并追踪派生资产；必要留存须说明依据与访问限制 |
...
上述事件7日、个人聚合30日、画像7日仅为历史合成开发假设，不作为真实服务TTL默认值。retention_policy_id引用由G批准的用途/期限配置；未批准不得开始对应持久采集。
