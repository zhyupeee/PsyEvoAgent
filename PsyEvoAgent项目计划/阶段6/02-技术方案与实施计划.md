...
候选生命周期使用candidate→offline_validated→human_approved→staging_validated→active，拒绝/研究保留/失效/回滚另记。human_approved允许指定hash进入staging；正式release必须同时校验staging证据、未过期批准、获准发布范围与当前数据资格。若staging更改hash，重新独立评价/审核。canary/production是release范围与实际证据，不从旧大写状态自动推断。未上线资格可停在研究/预发布资产。
...
| R02跨会话归因→记忆真实使用 | 不能只靠更长context宣称学习 | 保留清空上下文后的on/off；修改为授权/确认/遗忘链；不复制Hermes自动写权 | DependencyEvidence、memory_sources、tool_receipts | same-token no-retention/干扰/旧版/更正对照；阶段2/5/7 |
| R07局部delta→保留经验 | 更新一条策略时避免覆盖有效条目 | 保留局部diff/去重；加入来源、TTL、owner和审批；不在线写共享技能 | PolicyVersion.skill_entries、provenance | 同token条目检索 vs 全量注入、过期条目负例；阶段6 |
