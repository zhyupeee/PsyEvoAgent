...
状态回执为不可猜测的随机凭据，服务端只存哈希，范围锁定为一条删除任务的只读状态；不重新授予普通身份。有效跟踪期不得短于已告知的备份处理期，期限在阶段发布前配置。丢失/过期的回执走明确的支持核对流程，不以公开任务ID提供私人状态。凭据不进入URL、日志或分析事件。
...
备份的保留期限、RPO/RTO和导出TTL必须由数据/工程责任人在发布前配置并写入用户说明；未配置视为发布阻塞。删除留痕不能只有随数据库快照一起回退的表：使用既定私有对象存储的独立追加式清单，保存最少对象ID/范围、请求ID、状态回执哈希/有效期和可核对水位，不存正文，不随数据库恢复覆盖。对外确认删除请求已接受前取得该清单的持久化确认；失败不能声称请求已完整受理，保守停止相关访问并显示待处理。业务表与外部留痕按请求ID幂等对账；恢复缺失任务行时可由该清单重建最少状态及回执验证信息。
...
作业入口冻结case/schema、dataset_snapshot、source/purpose/grant/consent版本、split_manifest、candidate hash、VersionBinding、EvaluatorVersion、预算与访问额度，并记录fake/recorded/live证据域。规则、人工、Judge、Ragas结果分列；Reviewer意见不是独立真值。人工关键/争议集双人独立盲评后裁决，保留分歧/顺序和长度偏差。生成器只获允许的dev/select反馈，不得请求final；一次锁定终评作业按job_id/protocol_hash去重，重试不重复扣放访问额度或把未知结果当pass。
...
S5-A08核对观测与并行耗时；S5-A09核对隔离恢复后删除/撤权最新水位、失效会话/导出/事件正文及旧generation；S5-A10核对独立Eval/CI与资格；S5-A11仅Redis触发后执行。代价为评分/人工与恢复演练，失败保持原批准策略、暂停晋升；缺外部资格不得公开服务。
...
迁移采用expand→兼容应用→回填/校验→切换→另批contract；实际数据库/驱动/pgvector版本由托管支持矩阵确认。Alembic autogenerate/check只辅助发现差异，不证明数据转换安全。应用回退只切镜像和兼容策略指针，不能默认执行破坏性downgrade或恢复旧DB覆盖用户新数据。备份恢复沿原独立删除清单、水位、旧授权/导出失效及隔离放流量门禁；RPO/RTO/TTL没冻结不得声称生产就绪。
...
|---|---|---|---|
| G真实责任与发布 | 独立评审/预算/发布身份，年龄/专业支持/适用规范和研究资格 | 保留未就绪，不能宣称值守、转介或已通知 | 真实试用、公众服务、研究和发布；不代替适用资格审核 |
...
以下承接RSI总体第9节历史适用性核查，来源核查日期2026-09-17。本轮未重新核验法规现行状态或作法律认定；实施时由负责机构确认适用性、有效版本与证据。

**中国境内的适用性核查（规划依据，不替代负责机构法律/伦理认定）：**
- [个人信息保护法](https://www.samr.gov.cn/wljys/gzzd/art/2023/art_3ef1e889c1e644d4b65b5f5c7f432386.html)第14～19、28～31、44～47条支持目的/同意、敏感信息、未满14岁及查阅更正删除设计；跨境模型调用不能仅凭有API key就认为合法，须另核接收方、地区、用途和适用程序。
- [人工智能拟人化互动服务管理暂行办法](https://www.cac.gov.cn/2026-04/10/c_1777558395078289.htm)2026-04-10公布、2026-07-15施行。第2条适用于境内公众持续情感互动，本项目公开形态可能落入，不能凭“研究/非医疗”自动排除。第8/10/18/19条涉及诱导依赖、AI身份提示、连续使用每超过2小时提醒和便捷退出；2小时是该条法定提醒口径，不是本项目依赖诊断阈值或强制冷处理。
- 同办法第12～14、22、26条涉及年龄/联系信息、极端情境联络、未成年人模式、安全评估与备案。**这与旧稿“无自动联络能力”形成待关闭的上线条件**：本轮不新增发送能力、不联系任何人；未来若适用，须由机构明确合法联络依据、可信联系人、人工应急责任和安全替代，不能以保密承诺消除法定义务，也不能让Agent擅自猜联系人。未建立对应能力及审查前不开放适用公众服务。
- [涉及人的生命科学和医学研究伦理审查办法](https://www.nhc.gov.cn/wjw/c100375/202302/902b4a1dc3af4aba862a6387e6e376dc.shtml)2023-02-18发布，第2～3条覆盖境内高校等机构以心理学方法及人的信息数据开展的相关研究。长期支持干预/敏感行为研究由所属机构判断伦理路径；不能以线上问卷旧同意或模拟验证替代。

不假定大学生都成年。开发仅合成身份；首轮真人研究拟限定可核验成年且依法合规的同意范围，年龄未知/未成年不进入该研究或长期画像试用，提供适龄的静态帮助说明与依法审定的基础支持路径；正式未成年人服务须另完成人工治理和适用模式。安全支持不依赖依赖评分。现实联系人可能不安全，允许用户表达不宜联络者；不编造热线、可用时间、值守或已转介事实。

基于上述历史依据列出的固定AI身份/时长规则，与可选休息提醒、画像及安静时段分开实现；若适用性或版本未确认，记公众服务资格未关闭，不以工程通过替代。数据类别及保留期限见[保留合同](../阶段2/02-技术方案与实施计划.md#retention-contract)。阶段5基础交接不要求先对公众开放，真实服务与真人研究仍各自满足资格门槛。
...
| 记忆污染、错误画像固化 | 候选确认、claim类型、来源版本/反证/更正、到期；个人库与共享技能隔离 | stale并撤回关联产物；阶段2/3 |
...
| 成年范围、试用及必要伦理安排 | 项目/研究负责人，真实试用前 | 保留成年假设；不招募、不把原问卷授权扩展至应用 |
| 数据/媒体/案例许可 | 内容与工程，复制或入库前 | 不搬未知资产；保留源代码许可声明与改动清单 |
| 云平台、备份期限、RPO/RTO、容量与SLO | 工程/数据负责人，阶段5发布评审前 | 开发装配不能称最终生产；先测量再冻结门槛 |
...
| 专业内容/现实支持/年龄/适用法规 | 尚无足够真实上线前提，不承诺救援或自动联系 | 机构认定适用性、责任/联络/未成年人方案、安全评估/备案等实际证据 |
