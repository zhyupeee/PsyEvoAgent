...
> 整合修订：2026-09-21｜规划文档及S1-STEP02工程骨架；产品业务未实现，业务验收未执行。
> [项目导览](../../README.md) · [阶段目标](01-阶段目标与功能设计.md) · [技术方案](02-技术方案与实施计划.md) · [测试验收](03-测试与验收标准.md) · [分步实施](04-分步实施与检查清单.md)

除本页S1-STEP02明确列出的工程骨架外，业务代码路径仍为规划；本阶段业务验收未执行。规则以[共同合同入口与实施证据边界](02-技术方案与实施计划.md#missing-sources)为准。来源：S01生产架构，S02 V5–V12/V17的契约、流、工具及预算；R03练习交互、R12会话组织仅作适配参考。
...
| Query | 缺失但当前无需引入 | health.ts集中fetch、AbortSignal及组件局部状态足够；业务服务端数据出现后由Query负责缓存、失效和mutation，不复制到Store |
| Form / Zod | 缺失但当前无需引入 | 当前无表单，健康响应已有严格校验；新业务表单使用Form + Zod，DTO/Search Params按需复用Zod |
...
业务页面出现后，默认链路为route → feature → query/form/local state → service或Start Server Function → FastAPI/data source；保持route薄，但不为当前小型工程检查页强行拆目录。当前无QueryClient、Form、Store、Context状态入口或Server Function消费者，不创建空Provider。Loader、Server Function与Query按边界组合，HTTP薄客户端不复制Query的缓存、重试和loading职责；仅在Authorization、refresh、错误映射或trace等真实需求出现时增加。
