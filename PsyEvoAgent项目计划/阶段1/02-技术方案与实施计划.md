...
> 整合修订：2026-09-21｜STEP02工程与STEP03合成身份/授权已实现并局部验证；完整支持产品及业务整项验收未完成。
> [项目导览](../../README.md) · [阶段目标](01-阶段目标与功能设计.md) · [技术方案](02-技术方案与实施计划.md) · [测试验收](03-测试与验收标准.md) · [分步实施](04-分步实施与检查清单.md)

除本页STEP02工程、[STEP03实际实现](#s1-step03-implementation)和[STEP04隔离合成运行](#s1-step04-implementation)外，代码路径仍为规划；整阶段业务验收未完成。STEP04真实Provider/内容审阅仍BLOCKED。规则以[共同合同入口与实施证据边界](02-技术方案与实施计划.md#missing-sources)为准。来源：S01生产架构，S02 V5–V12/V17的契约、流、工具及预算；R03练习交互、R12会话组织仅作适配参考。
...
前端采用React/TS、TanStack Start/Router/Query/Form与Tailwind CSS；保留原生语义控件，Tailwind utility与CSS-first主题token实现布局和视觉。Start处理页面服务，网关同源 `/api/v1` 转发FastAPI；公开介绍页SSR，私人页面客户端取数。后端Python 3.12兼容环境、uv锁依赖、FastAPI/Pydantic、异步HTTPX和SQLAlchemy/Alembic。工程验收前锁定实际版本，不把上游宽泛版本范围当兼容证明。
...
沿原FastAPI→Meta→LangGraph→Support→Output Policy。新增版本化实验默认配置与来源用途映射、InteractionEvent接收器与AgentBehaviorEvaluator规则函数；它们不是自治Agent。Meta控制合法资料、固定支持边界、调用预算与取消。无需每轮LLM分类，语义不确定时先自然澄清。
...
flowchart TB
 ...
 JOB --> MC[Memory candidate / 校验后自动业务写入]
 JOB --> OPT[L1或隔离L2候选]
...
<a id="s1-step04-implementation"></a>
### STEP04 隔离合成运行实现合同（2026-09-24）

本步消费者为后端内部调用和确定性验收；尚无start/SSE/聊天页面消费者，不提前接STEP05/06。复用STEP03身份/来源事实，STEP04的输入由可信调用方提供owner、session/run、消息版本及来源快照，调用前/每次重试/输出前检查一致性和有效性。当前仅合成测试装配，不把内部类型当成客户端可授予身份的接口。

唯一图：`START → meta → support → output_policy → END`。Meta为固定规则，优先当次拒绝/结束/安全诉求；意图含糊保守澄清，不增设独立分类LLM。allowed_tools为空（本步无可调用的已审资源消费者），任何模型tool调用或伪称写入均拒绝；不建第二Agent或Worker消费者。LangChain core承担消息与fake模型适配，LangGraph承担唯一执行图；尚未选定的真实厂商SDK不安装。

运行版本固定为`support-graph/1`、`support-policy/1`、`behavior-rules/1`、`support-runtime/1`及现有`experiment-defaults/1`；assessor显式`disabled`，不虚构画像评分器。配置不可热换，语义评分为null；规则通过不等于独立语义/专业审阅通过。所有候选完整缓冲，结构化schema与固定规则均通过才返回正文；无已审回退内容时Controlled Stop，正文为空并返回明确原因。

合成预算基线`s1-fake-budget/1`：最多2次调用、8192总token、每次最多1024输出token、10秒原始deadline、3个图节点；预占输入UTF-8字节数加输出上限（仅fake保守计量，不能冒充真实模型tokenizer）。合成价格`fake-price/1`为每token 0.001 SYNTHETIC，费用上限8.192，绝非供应商报价。真实usage可信时结算，缺失/不可信时actual=null并保留全部预占，停止后续调用；超预算返回失败不退款。成功、错误、取消、重试均留run/request/attempt/model/版本/耗时/预占/actual或unknown元数据，不留正文或异常原文。fake账本随验收归档；业务持久调用表和run启动原子绑定承接STEP05，不新增第二套run存储。

只有未产生正文的429/瞬态故障可统一重试；Retry-After需有限非负且不越原deadline，每次都重新预占并占调用次数。schema错误不自动修复（本版修复配额0），拒答、截断、半截流、工具、usage未知及取消不重试。SDK/图/Worker隐式重试均不得叠加。当前仅允许LangChain的本地fake模型，显式关闭追踪外发，不读取Provider密钥；真实模型、价格、数据条件、取消/usage能力继续unknown/BLOCKED，不能将fake验证标为真实tested。
...
| consent_records | 仅保留历史用户决定及原告知版本；新实验默认配置不生成granted记录，不作为业务前置 |
...
阶段1流只使用 `run.started`、`message.delta`、`run.completed`、`run.failed`、`run.cancelled`；状态事件附run_version，artifact与approval事件留到真实消费者阶段。取消与完成以服务端终态CAS先成功者为准；前端收到冲突先取终态，仍活动时可重发取消。断网、身份过期、代理缓冲、重复事件及终态竞态有专门测试；浏览器停止需通知服务端，不能只停止绘制。draft可取消，不设固定到期，不发送模型、不产生假消息；未带额外资料的普通聊天可走便捷入口。
...
| 接口 | 输入→输出 | 校验/错误 |
|---|---|---|
| 身份注册、登录与密码管理（目标） | 邮箱验证码注册、邮箱密码登录、验证码找回和已登录改密 | 见[邮箱身份目标合同](#email-identity-target)；S1-STEP03.5待实现，当前用户名接口见[当前合成实现](#s1-step03-implementation) |
| GET `/api/v1/experiment-config` | 已登录→version/defaults/implemented/user_consent | experiment-defaults/1；五类功能默认开启与实际实现分开；user_consent=false；旧consents接口仅兼容历史，不由当前客户端调用 |
| PATCH `/api/v1/me/preferences` | 支持方式、显示及expected_version→新版本 | 历史age_band/age_source不改写；新账号unknown/not_provided；客户端无年龄选择 |
| POST `/api/v1/interaction-events` | event_id、session/run、type、时间、initiation、可选active_ms/coverage→accepted/duplicate/rejected计数 | 服务端归属、有限batch、时钟偏差、重复键、非法负时长校验；来源不可用拒绝 |
| POST `/api/v1/feedback` | run_id、helpfulness、category、optional_comment→feedback_id | 内部评测按实验配置处理，不随反馈附会话原文，不将点赞作为政策晋升票 |
...
#### STEP05实际装配合同（2026-09-24）

实现路径为`backend/app/runs.py`、`run_stream.py`、`run_worker.py`，复用原Run、身份/CSRF、来源关联及唯一SupportRuntime。新增`run_executions`（每run唯一的启动绑定/预算/deadline/generation/事件序号，不另存status）、`messages`、`run_events`、`model_calls`、`interaction_events`。业务终态仍以runs为准；增量迁移`d642b94fcbb5`保留原记录，已有本步数据时拒绝有损降级。

start正文为`input.message`（非空、最多4000字符）、`client_message_id`、`expected_version`、`expected_session_version`、`grant_ids`（最多8个、不重复）、`initiation=user/requested_followup/unknown`。同owner的Idempotency-Key与路径/请求hash绑定，重放核当前来源并返回原run，同key异内容409。便捷POST sessions/{id}/runs仅启用kind=message，相同client_message_id/内容返回原run；regenerate/历史修订属于STEP07。start原子保存当前消息、版本绑定、固定fake预算和queued状态；draft、GET及SSE零模型调用。grant绑定来源关联，不因此实现额外历史正文检索。

只有`PSYEVO_ENV=test`、独立loopback合成check库且`PSYEVO_SUPPORT_MODE=fake`时允许执行；默认disabled返回503 provider_not_configured，不读取模型密钥。独立`python -m app.worker --support`串行消费，API导入/reload零消费者。owner行锁及条件更新串联claim与终态CAS。queued时冻结10秒deadline，断流/重试不重置；过期running记interrupted/run.failed，不重新执行。每次调用前、重试前、输出前及持久提交核来源消息版本/删除、grant、身份和generation。调用预占先持久化，取消/未知usage保留预占，迟到结果不覆盖终态。

GET run返回获准output、version_binding、budget/budget_used、deadline_at、generation及last_event_id；delta与completed在同一事务提交。draft可取消但不产生模型事件；活动版本冲突409，终态返回已提交事实。退出/改密停止对应活动run，Worker也复核身份/来源失效。业务保存期限无固定TTL，10秒仅为一次执行预算。

锁定FastAPI 0.141.1的原生EventSourceResponse/format_sse_event负责唯一编码，无新增依赖。十进制游标query/header冲突或超前422，过期410，恢复只读同run快照。每run保留最新128个公共事件（数量窗口），序号不重置；每次发布取一个事件并核权，1秒注释心跳、32768字节单帧上限、2秒发送超时、当前单API进程每owner最多2条流。无无界生产队列；多进程全局配额未扩展。本步验证现有Vite同源代理，不冒充远程Caddy或公开发布。

互动批次最多50个，event_id/hash与tab/sequence去重；真实发送由服务端唯一client_message_id记录，重连/模型重试/心跳不增计数，可选互动记录失败不阻断基本启动。active_segment包含tab_id、sequence、单调start_ms/end_ms、visible/focused，区间最多30秒；客户端时间与接收时间偏差超过5分钟拒绝。以报告的occurred_at对齐后取跨标签并集，排除闲置/后台段，最多核最近1000段并显式返回truncated。reported_union_ms仅为报告区间合并值；未校准真实设备/无障碍误差，故active_ms=null、coverage=uncalibrated，不冒充注意力测量。system_push只保留报告/合成来源标签，不实现推送。

本节描述实现，执行状态见[STEP05记录](evidence/S1-STEP05/README.md)；STEP06页面、STEP07删除清理及真实Provider/内容审阅不在本步范围。
...
<a id="s1-step02-environment"></a>
### S1-STEP02 环境合同与实际盘点

环境合同版本 `s1-environment/3`，2026-09-21。接续已有未提交的工程骨架实施；不再使用早期“无源码”的阻塞原因。实际启动、依赖准备及检查命令统一见[本地开发说明](../../DEVELOPMENT.md)，结果见[本步执行记录](04-分步实施与检查清单.md#s1-step02-evidence)。本步不实现身份、持久化或 Agent。

| 对象 | 实际入口/版本 | 实施边界 |
|---|---|---|
| Python | backend/.python-version=3.12.13；pyproject.toml + uv.lock；uv 0.11.7 | FastAPI 0.141.1；HTTPX/Pydantic/Uvicorn；dev组含Ruff/mypy/pytest/psutil |
| 前端 | Node 22.19.x；pnpm 10.28.0；package.json + pnpm-lock.yaml | React 19.3.0、Start 1.168.56、Router 1.170.38、Tailwind CSS及Vite插件4.3.3；实际脚本包含dev/build/lint/typecheck/test:unit/test:e2e；审计补齐Prettier 3.9.8及format/format:check/test/check |
| API | backend/app/main.py:create_app；GET /api/v1/health | 返回严格的 status=ok、stage=S1-STEP02；仅工程存活检查，不代表数据库/模型就绪；无业务身份或写入 |
| Worker | backend/app/worker.py，显式独立入口 | 生命周期探针，consumers=0；无任务/队列；API不导入、不启动它 |
| 前端网关 | frontend/vite.config.ts | 开发时 /api/v1 转发127.0.0.1:8000；无生产网关部署 |
| 前端样式 | frontend/src/style.css；frontend/src/routes/*.tsx | Tailwind由Vite插件编译；CSS-first主题token加utility classes；无PostCSS或独立Tailwind配置文件 |
| 配置 | backend/app/config.py；Vite envDir=false；根.gitignore | API/Worker仅读PSYEVO_ENV，不自动加载.env；校验development/test；秘密不得进前端 |
| 检查 | scripts/check_step02.py；scripts/check_step02_isolated.sh；scripts/probe_os_isolation.py；Python/Node网络钩子 | 安装与执行分离；Windows开发回归、Linux/WSL内核级PR出口隔离；独立收据、外连探针及隔离前置失败即停止；非完整文件系统沙箱 |

API import只定义应用工厂，lifespan负责当前进程资源；目前无HTTP/DB长生命周期客户端。本地开发通过uv运行backend/app/dev.py，复用已锁定的watchfiles与Uvicorn Server API，使用进程事件请求优雅退出后重载；监督模式以非阻塞stdin EOF触发停止，避免Windows阻塞管道读取卡住子进程初始化。Windows与WSL均已通过实际导入、连续两次reload、三次启停及无残留子进程测试；旧Uvicorn原生reload失败收据保留。Worker单独启动退出及协程取消测试不替代API reload。

PR门禁通过Linux/WSL私有user/network/mount/PID namespace仅保留loopback，移除capabilities并禁止提权；WSL私有挂载阻断/init互操作解释器，宿主保持不变。完整门禁执行前用不加载语言网络钩子的探针验证IPv4/IPv6 TCP/UDP、原生curl及Windows互操作拒绝；没有内核隔离则拒绝开始测试，不能把普通入口当作PR隔离验收。命令、前置依赖和配置读取见[开发说明](../../DEVELOPMENT.md)，收据见[局部验收](03-测试与验收标准.md#s1-step02-validation)。

fake Provider仅在backend/tests/fakes.py提供固定响应/异常，验证结构化校验和异常传播；不实现STEP04的Runtime、重试和预算。浏览器测试覆盖工程页键盘、停止、错误焦点及移动布局；不替代授权/撤权/业务run取消。当前无数据库消费者，因此未创建Compose、迁移或数据库配置；需要时默认只启PostgreSQL。

本次影响README、DEVELOPMENT、AGENTS、阶段1的02/03/04、前后端工程、检查脚本及本步证据；原STEP01收据保留。

<a id="frontend-stack-contract"></a>
### 前端职责与按需接入合同

2026-09-21前端审计确认：React + TypeScript + TanStack Start/Router为现有主线；参考项目只提供补充能力，不能替换主线。判定优先级为当前仓库用法与约定、TanStack官方职责、参考项目独有能力、未来候选。新增依赖必须同时满足“当前功能需要、没有等价实现、属于确定主线”；用户已明确选择严格按消费者补齐，UI基础也不例外。

| 能力 | 当前状态 | 现有消费者与后续触发条件 |
|---|---|---|
| Start / Router | 已对齐 | src/router.tsx工厂、文件路由、生成路由树；Start Vite插件负责框架装配，不另建SSR或路由系统 |
| Tailwind v4 | 已对齐 | 唯一src/style.css与现有页面；保留CSS-first主题和Vite插件 |
| Query | 已对齐 | STEP03账号页由Query维护身份、请求及mutation；health.ts仍用集中fetch与局部状态，不复制到Store |
| Form / Zod | 已对齐 | STEP03注册/登录使用Form + Zod，账号响应由Zod校验；DTO/Search Params按消费者接入 |
| Store | 缺失但当前无需引入 | 无真正共享客户端状态；跨页面草稿等消费者出现后评估，简单状态继续用React局部状态 |
| shadcn / cn / Lucide / Sonner | 缺失但当前无需引入 | 当前原生控件足够；真实UI需要时接components.json、统一cn及必要组件，不批量生成组件 |
| ESLint / Vitest / Playwright / Prettier | 已对齐 | 工程源码、2项单元测试、4项E2E；新增Prettier 3.9.8及format、format:check、test、check脚本 |
| MSW / jsdom / Testing Library / Git hooks | 缺失但当前无需引入 | 当前fetch stub、Playwright拦截及显式门禁足够；出现对应测试或提交自动化需求再评估 |
| Table / Virtual / Pacer | 未来保留 | 分别由复杂表格、大量消息或真实节流/排队需求触发，不预建wrapper |
| DB / AI | 未来保留 | 明确客户端同步数据库或TanStack AI消费者后才评估，不因项目名称含AI而安装 |
| 图表 / Markdown / Highlight / Hotkeys | 未来保留 | 当前无消费者；图表选型未决，接入时核实候选官方能力、版本和真实需求，不同时安装三套图表；稳定既有实现不为改名而迁移 |
| XYFlow / Dagre / Monaco / 动画 / 3D | 未来保留 | 分别由工作流图、编辑器、动画或WebGL页面触发；只选所需方案，动画须正确清理 |

业务页面出现后，默认链路为route → feature → query/form/local state → service或Start Server Function → FastAPI/data source；保持route薄，但不为当前小型工程检查页强行拆目录。STEP03已有账号页QueryClient和Form消费者；Store、额外Context或Server Function仍无消费者，不创建空Provider。Loader、Server Function与Query按边界组合，HTTP薄客户端不复制Query的缓存、重试和loading职责；仅在Authorization、refresh、错误映射或trace等真实需求出现时增加。

新代码不引入Zustand、react-hook-form/resolvers、Axios、react-router-dom、Next.js、自建Vite SSR、nprogress或第二套主组件/图标库来覆盖已有职责。发现既有冲突库时先列文件、状态归属、消费者、迁移成本和业务风险，未经授权不卸载。当前未发现这类业务依赖。接入shadcn时CSS须指向真实入口，rsc依据实际能力判断，不能把SSR等同于RSC；不并装重复primitive体系。

版本以package.json与pnpm锁文件为准，当前直接依赖声明一致；Start与Router版本号不同不自动视为冲突。仅在兼容失败、明确安全问题、锁不一致或已定方案无法满足时改版，不批量追新。此次安装提示ESLint 9.39.5已不受支持，保留版本且lint实测通过；未执行安全专项核验。官方生成的routeTree.gen.ts保留生成器类型抑制，禁止手改；手写代码不得新增any、as any或@ts-ignore绕过检查。

日常命令及格式化范围统一见[DEVELOPMENT](../../DEVELOPMENT.md)；`check`不包含build/E2E，也不替代scripts/check_step02.py或Linux/WSL隔离门禁。审计结果只允许使用“已对齐、缺失需立即补、缺失但当前无需引入、未来保留、多余 / 冲突、版本偏离、需要确认”；没有消费者不等于实施缺陷。实测边界见[补充实施记录](04-分步实施与检查清单.md#frontend-audit-followup)。

### 最小工程门禁与异步资源

关联任务：S1-T02、S1-T03、S1-T08；验收：S1-A13。

Python先锁一个3.12兼容环境，实际依赖按API/Worker/离线优化需要分组；不使用“3.12+”作为无限兼容承诺。本步已使用uv和pnpm分别管理后端与前端；若取得的真实仓库已有可用锁文件，则先保留，不无条件迁移。Ruff负责lint/格式检查，mypy检查业务边界和schema类型，pytest覆盖有业务含义的失败路径；不增加与其重复的默认检查器。
...
以下命令仅在实际backend/frontend目录、依赖锁文件、脚本和隔离配置存在后运行；STEP02实际入口及结果见上节；下面业务迁移/运行事件/任务恢复模板仍未落地。Python/前端任一授权、类型或取消回归失败即阻止该切片交付。PR无外部模型出口，只使用合成fixture和受限测试数据库。

```bash
# 尚未实现：仅在业务迁移、测试路径及隔离数据库存在后执行
...
增量任务按下表登记；原S*-T*含义保持，页内Txx为本阶段基础任务的局部简称。每个工作包S/M（规模定义见[预算口径](../阶段6/02-技术方案与实施计划.md#cost-assumptions)），不承诺工期。阶段2复用source/version/实验配置快照；本阶段不创建长期画像表的生产数据。
...
|---|---|---|---|---|
| RSI-S1-T01 <a id="rsi-s1-t01"></a> 冻结支持边界与合成反例 | [S1-T01](02-技术方案与实施计划.md) | 本阶段前置门槛 | U01/U04；实验配置、固定支持规则与合成case | [RSI-S1-A01](03-测试与验收标准.md)、[RSI-S1-A05](03-测试与验收标准.md)；共同S1-A11、S1-A12、S1-A13 |
| RSI-S1-T02 <a id="rsi-s1-t02"></a> 定义实验配置/事件schema及账号UI | [S1-T02](02-技术方案与实施计划.md)、[S1-T05](02-技术方案与实施计划.md) | RSI-S1-T01 | U01/U02；历史ConsentRecord/InteractionEvent、experiment-config及注册登录账号页 | [RSI-S1-A02](03-测试与验收标准.md)、[RSI-S1-A03](03-测试与验收标准.md)、[RSI-S1-A04](03-测试与验收标准.md)；共同S1-A11、S1-A12、S1-A13 |
...
| 前端框架 | React、TanStack Start/Router/Query/Form、Tailwind CSS；公开SSR、私人客户端取数 | Table/Virtual依实际消费者，浏览器E2E工具在实际仓库中选定并锁版 | Next.js替换、第二身份/session/run store、并行第二套样式框架 |
...
默认保持上述主线：LangChain承担模型、Tool、Retriever等能力层，LangGraph是唯一主要Agent/Workflow Runtime；Multi-Agent优先用同一Runtime的Node、Subgraph、State、Command、Send、interrupt和checkpoint表达；PostgreSQL是业务事实库，pgvector是当前生产向量检索方案。CrewAI、AutoGen、ChromaDB、Milvus及其他重叠基础设施不是因为流行、招聘要求、Stars或“技术栈更丰富”而加入。

每次架构复杂度上升时，先记录“问题→证据→候选PoC→对比→ADR→决定”：区分实现问题与能力缺口，说明当前方案是否仍能合理解决，并核算第二套Runtime、数据库、Workflow或职责边界的长期成本。没有真实规模、延迟、资源或维护证据时，不迁移、不并行维护重复基础设施。

CrewAI只在角色团队协作成为主要模式，且LangGraph出现大量重复角色/任务/委派/汇总代码、样板和调度复杂度已形成真实痛点时建议做隔离PoC；对照当前LangGraph实现，至少比较开发复杂度、状态、可观测性、调试、HITL、checkpoint/resume、Tool、Memory、测试、错误恢复、性能和维护成本。AutoGen只在Agent间自由对话、协商、反驳、轮次与终止控制成为核心且手工消息路由明显失控时建议PoC；至少比较对话自然度、路由和状态可控性、循环风险、Token成本、Debug/Trace、HITL、恢复、测试及与LangChain/LangGraph兼容性。两者均不得直接替换LangGraph，结论须进入ADR并明确最高层Runtime及State、Checkpoint、Resume、Retry、Trace、Memory、Tool、HITL、错误处理和生命周期归属。

ChromaDB仅可作为不依赖完整PostgreSQL的轻量、隔离、本地RAG实验组件，实验结束不得自动替换生产PostgreSQL+pgvector。只有在真实向量规模、P50/P95延迟、并发、资源占用和pgvector调优结果证明独立向量层需要独立扩缩容或已影响业务负载时，才评估Milvus、Qdrant、Weaviate等候选；评估须基于真实数据和基准，不以“AI专用向量库”作为理由。任何候选引入都要说明谁拥有业务状态、向量索引、检索、回退、删除、观测和恢复职责，避免形成多套Runtime或数据真相。

该判断是持续架构门禁：发现LangGraph协作重复、自由对话增多、RAG规模或pgvector性能下降、或MCP/Tool出现现有方案无法解决的缺口时，主动提出是否做隔离PoC；证据不足则继续当前主线并记录待测指标。
...
| 实验数据保留、来源与操作责任 | [阶段5治理](../阶段5/02-技术方案与实施计划.md#service-governance) | 删除、外发确认及恢复证据；无固定业务TTL、公众资格或期限审批前置 |
...
| C05 | 画像按实验配置默认开启；无有效来源或缺失证据时回退当次上下文；当次诉求、拒绝和纠正优先 | [阶段3适配](../阶段3/02-技术方案与实施计划.md) |
...
具体Provider、实际SDK/依赖锁、中文校准与外部MCP需求按各阶段真实消费者核定；不因文档修订而标为已实现。本次不实际发布远程服务，不新增服务或研究资格审批。
...
<a id="s1-step03-implementation"></a>
## S1-STEP03 当前合成实现合同

**当前调整：** 内部实验取消consent、run draft和context_grant的固定到期时长，新增迁移d031_experiment_expiry使expires_at可空；API返回null，历史非空值保留但不再参与业务核权。登录会话8小时安全失效保持。有效性由owner、用途、撤回、来源版本/删除和run状态决定。此前两次迁移及收据为调整前历史，数据原样保留。

2026-09-21；以下为本步实际实现，覆盖此前 STEP01 的缺口映射中身份、consent/grant、session/run/偏好与删除状态部分。STEP01 映射和 STEP02 收据保留历史含义。代码落点为 `backend/app/models.py`、`contracts.py`、`api.py`、`security.py`、`provision.py` 与 `backend/migrations/`；页面为 `frontend/src/routes/me.privacy.tsx`。完整工程装配和配置读取见 [DEVELOPMENT](../../DEVELOPMENT.md)。

### 本地身份及启用边界

采用持久化opaque会话，Cookie存随机令牌，数据库存SHA-256摘要、CSRF token、到期与撤销时间；密码采用随机盐PBKDF2-SHA256/600000次。当前注册须先验证邮箱验证码，再以至少6位密码创建账号、偏好和8小时会话。登录仅用邮箱密码；旧用户名数据由迁移保留但不再作为登录身份。5次错误密码暂锁1分钟，登录/注册旋转会话，退出撤销。写请求准确校验单一Origin与CSRF；注册/登录同样检查Origin。响应no-store，错误不回显密码或数据库异常。

API接受显式PostgreSQL地址与库名，不限本地或synthetic前缀，未配置返回503。environment=test及自动化验收只允许独立loopback合成check/migration库，拒绝开发库。owner由认证身份注入；跨用户404，版本冲突409，写入事务提交后才成功。幂等重放重新检查当前资源，不复活失效授权。

| 本步端点 | 当前行为及边界 |
|---|---|
| POST `/api/v1/auth/register`、`/auth/login`；GET `/auth/session`；POST `/auth/logout`（均在/api/v1下） | 邮箱验证码注册后自动登录，邮箱唯一冲突409；安全Cookie、CSRF与退出；旧账号和原哈希由迁移保留 |
| GET/PATCH `/api/v1/me/preferences` | 正整数 expected_version；mode、display_preferences、age_band 持久化；unknown默认，adult/minor仅标self_reported |
| GET `/api/v1/experiment-config`；历史 `/api/v1/consents` | 版本化默认配置与历史用户决定分开；支持/记忆/画像/内部评测/优化默认true，实际能力单独列举；当前客户端不调用consents |
| POST/GET/PATCH `/api/v1/sessions`（GET/PATCH带ID） | 本步数据CRUD切片：创建、中性标题、本人读取及版本化标题更新；历史搜索/归档界面留 STEP07 |
| POST `/api/v1/run-drafts`；GET `/api/v1/runs/{id}` | 仅创建message draft，绑定会话版本，不设固定到期，不读消息、不启动模型；start/cancel/SSE留 STEP05 |
| POST/GET/DELETE `/api/v1/context-grants`（GET/DELETE带ID） | conversation/current_run；POST无需且不接受consent_id；服务端绑定owner/source/version/draft，重查删除及终态；未来由业务动作内部建立，不弹用途授权框 |

历史consent及年龄原样保留，不参与新实验入口或业务执行校验。新grant的consent_id为空、consent_refs为空列表，不伪造granted。默认开启仅为版本化项目配置，尚未实现的记忆、画像、评测与优化不产生虚假功能入口或运行结果。

### 持久 schema 与迁移

实际表为 email_codes、users、identity_sessions、user_preferences、consent_records、conversations、runs、context_grants、deletion_jobs、idempotency_records。个人对象采用共同封套的整数 schema_version=1、opaque UUID、UTC带时区时间、owner、版本、来源/consent引用与 synthetic-local/1 保留标识。run未执行时 source_message_version 为空，不伪造消息或 VersionBinding；运行字段由 STEP04/05 在启动前补全。

基础迁移7af8981db341和owner约束c1756470d092保持不变；追加d031_experiment_expiry取消固定业务TTL，e032_experiment_defaults使grant.consent_id可空，旧引用与owner复合外键保留。迁移不改写账号、年龄、历史consent或旧grant。含新无consent记录时拒绝有损降级。deletion_jobs目前仅有持久模型，不代表完整删除已实现。

合成保留标识、告知版本和自报年龄仅供本地验证。当前内部实验不要求真人服务资格、正式年龄核验或保留期限审批，不作为STEP阻塞项。本步局部验收见[测试边界](03-测试与验收标准.md#s1-step03-validation)。

<a id="email-identity-target"></a>
### S1-STEP03.5 邮箱身份合同（已实现）

2026-09-23本节接口已实现，替代旧用户名入口；验证范围和未完成项见[本次独立记录](evidence/S1-STEP03/email-identity/README.md)，不覆盖历史收据。注册输入邮箱、邮箱验证码和密码；验证码通过后才创建用户与偏好，并自动建立8小时会话。普通登录只使用邮箱和密码，不重复发送验证码。忘记密码用邮箱验证码重置；已登录改密校验当前密码。新密码为6～128位且无字符组合要求，页面要求重复输入新密码。邮箱去首尾空白并统一大小写后校验，长度不超过254，按规范化值唯一；不做特定邮件服务商的别名或点号合并。目标页面沿现有TanStack Start/Router、Query、Form和Zod接入，不增加年龄、用途或同意前置。

| 当前端点（均在`/api/v1`下） | 输入→输出 | 约束 |
|---|---|---|
| POST `/auth/registration-codes` | email→202通用回执 | 只发验证码，不创建账号；已有邮箱不通过回执暴露 |
| POST `/auth/register` | email、code、password→201 user_id/email/csrf_token及安全Cookie | 验证码有效后事务性创建账号、偏好和会话；唯一邮箱竞态409 |
| POST `/auth/login`；GET `/auth/session`；POST `/auth/logout` | email、password→身份会话；session返回user_id/email/csrf_token | 登录不校验邮箱验证码；保留原Cookie、Origin/CSRF、同一规范化邮箱无论是否注册均在5次错密后暂锁1分钟、退出撤销 |
| POST `/auth/password-reset-codes` | email→202通用回执 | 账号不存在也返回相同外部结果，不泄露邮箱是否注册 |
| POST `/auth/password-reset` | email、code、new_password→204 | 验证成功才更新哈希，撤销该账号全部会话；用户重新登录 |
| POST `/auth/password-change` | 已登录，current_password、new_password→204 | 校验当前密码与CSRF，成功后撤销全部会话；用户重新登录 |

验证码按注册与重置用途绑定：6位随机数字、10分钟有效、最多5次校验、单次使用；重发间隔至少60秒，重发作废旧码，每邮箱每小时最多5次，全局每小时最多100次。只存带独立服务端密钥的摘要，不记录明文验证码或密码；错误码和日志不泄露邮箱是否存在、验证码或SMTP凭据。验证码期限仅为身份安全机制，不是业务资料TTL。安全Cookie、响应no-store、账号隔离和现有PBKDF2-SHA256哈希保持；全部未登录写请求校验精确Origin，已登录改密还校验CSRF。重置或改密不自动登录；向邮箱发送不含密码的变更通知。

SMTP由`app.config.load_settings`从显式进程变量`PSYEVO_SMTP_HOST`、`PSYEVO_SMTP_PORT`、`PSYEVO_SMTP_FROM`、`PSYEVO_SMTP_USERNAME`、`PSYEVO_SMTP_PASSWORD`、`PSYEVO_SMTP_TLS_MODE`（`starttls`或`ssl`）和`PSYEVO_EMAIL_CODE_KEY`读取，不自动加载`.env`。未配置发送能力时验证码入口统一返回503；真实凭据保持忽略或由部署环境注入，验收以隔离邮件替身替代外发。新增依赖以实际消费者为准，不引入模型或后台消费者来完成身份流程。

持久层仅追加可空邮箱列、唯一规范化邮箱约束、验证码记录和按邮箱保存的登录失败计数，允许新账号无用户名；迁移保留现有用户名、密码哈希及其关联记录，不在迁移中删除账号。切换邮箱专用登录前，仅对确认的本机`psyevo_synthetic_dev`安排一次独立、事务性的旧账号及关联实验数据清理，不在日常重启时清理。其他环境保持数据不动，须先制定迁移方案方可切换登录入口；现有`app.provision <username>`不能作为新邮箱账号的验证旁路。追加迁移f033_email_identity和g034_login_attempts已实现；SMTP读取与TLS发送已实现但未配置真实凭据。限定本机开发库清理及合成开发账号创建已执行，实际影响见新增记录。

补充执行规则：规范化邮箱当前接受ASCII地址（本地部分最多64字符、总长254）；含国际化域名须先使用ASCII域名形式。每邮箱和全局发码限额按过去一小时滑动窗口、合并注册/找回请求计算。数据库事务锁保护发码与单次消费，错误尝试单独提交；重发仅作废同用途旧码。对已存在和不存在邮箱均发送同类验证码，返回相同202；持有有效注册码后的重复邮箱返回409，已用码返回400。未知账号重置在有效码后同样204，但不创建账号。变更通知采用通用文案，不含密码；发送失败不回滚已提交的密码与撤销记录。普通API没有验证码查询或合成邮箱开关，测试替身仅由测试工厂注入。旧用户名provision命令明确拒绝创建。
...
版本化 experiment-defaults/1 控制 support、memory、profile、evaluation、optimization，均默认true；它不是用户同意。ConsentRecord仅作历史，consent_refs可空，业务不要求consent。来源用途仍区分service_context/current_run、optional_profile/dependency_assessment、saved_memory/candidate_extraction/personalization、research/research_export/evaluation；服务端依据业务动作和来源建立grant，模型和客户端不能授予owner权限。内部评测与优化仍须来源追踪、去标识和评测集隔离。

运行前原子绑定 `VersionBinding={policy_version, assessor_version, evaluator_version, graph_version, model_ref, schema_version, experiment_config_version}`，同一run不热换。每次读取、外部调用、handoff、写入和输出重新核权/来源版本；新版本仅新run使用。审批绑定资产hash与基线，hash变化失效；恢复检查幂等业务收据，不能只信模型日志。个人修正可使Profile立即stale，正在生成的关联输出停止。
...
记录默认仅本人可访问；context_grants绑定来源、版本、用途与run，由服务端业务动作建立，不新增授权弹窗。每次读取、提取、提交和重试检查owner、来源版本、删除状态、run和抑制标记。记忆提取校验后自动保存，可查看、更正及确认删除；不将模型推断标为用户确认事实。支持、画像、内部评测和优化不依赖用户consent。外发等重要副作用仍需动作确认，已发送内容不能因本地取消声称收回。
...
处理状态为 `requested → online_blocked → derivatives_purged → backup_pending → completed`，失败为 `failed_retryable`并显示待处理项。前端只有全部范围有证据才显示完成。无备份的早期开发阶段使用当前删除任务记录，备份项不适用，不依赖阶段4对象存储；启用任何包含用户数据的备份前必须建立独立清单，不能先开备份再补治理。备份不设固定业务保存期限，阶段5实现删除清单与恢复校验，此后删除请求接受前需取得独立于数据库快照的持久清单确认；恢复获取最新清单并核对水位，缺失时禁止接流量。账号删除撤销普通身份，同时提供只读本任务状态的受限回执，不恢复原数据访问。来源显式撤销停止相关使用；历史consent撤回不作为新实验开关，删除原始记录仍按明确范围确认。日志以元数据为主，必要保留和供应商删除能力列入数据说明。
...
|---|---|---|
| ConsentRecord | 历史purpose=service_context/optional_profile/saved_memory/research；decision=granted/declined/revoked；notice_version、scope、expires_at、revoked_at | 原记录保留，不改写成实验默认同意；不是当前业务执行前置；默认配置版本独立维护 |
...
| optional_profile → dependency_assessment；saved_memory → candidate_extraction / personalization | [用途映射](#shared-contracts)；画像、候选提取、自动保存的记忆、后续检索不能互授；阶段1不开这些持久能力 | 未实现；仅登记兼容方向 | 阶段2；本步不创建接口或表 |
...
| VersionBinding.policy_version/assessor_version/evaluator_version/graph_version/model_ref/schema_version/experiment_config_version | [版本绑定](#shared-contracts)：运行前原子绑定，不热换；未启用组件如何显式表示须在具体 schema 落地时决定，不伪造已批准版本 | 未实现；所有实际版本/模型 unknown | S1-STEP04、S1-STEP05 / 阶段1；后续阶段扩展 |
...
| [Tailwind CSS的TanStack Start安装指南](https://tailwindcss.com/docs/installation/framework-guides/tanstack-start) | 官方建议通过`@tailwindcss/vite`及根CSS导入接入；项目实际版本以锁文件为准 | 使用Tailwind v4 CSS-first配置，不并装PostCSS链或第二套样式框架 |
...
P03补齐邮箱验证码，校验通过后才创建账号；S01不作为年龄、用途或同意前置。找回、改密及错误状态按身份合同补齐，登录后仅进入已实现功能。

沿用[前端职责与按需接入合同](#frontend-stack-contract)：React + TypeScript + TanStack Start/Router保持主线，Query负责业务服务端状态，新业务表单使用Form + Zod，Store仅用于真正共享的客户端状态。shadcn/cn/Lucide/Sonner等仍由真实UI消费者触发，参考图不构成安装依赖、预建空组件或更换框架的依据。本次仅补充设计参考，不新增接口、schema、路由或依赖，不宣称页面已实现。
